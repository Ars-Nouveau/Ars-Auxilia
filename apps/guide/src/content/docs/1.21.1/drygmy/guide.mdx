...
- Entities with {"<"}100% chance to drop items from their loot table can generate "no loot" pulls into the Loot Table, determined each time the Drygmys finish a cycle.
...
- You need 20<sup>C</sup> Drygmys to max out a henge's production speed. In theory twice that many, with half of them desynced by 100 ticks would be better, but you'd need to create and maintain that desynch at all times. 
